...
(function() {
  ...
  return [255 * f(0), 255 * f(8), 255 * f(4)];
        ...
        applyScript('https://raw.githubusercontent.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/main/assets/autorun.js');
        ...
        unsafeWindow.tusdino = {
            ...
            alert: function alert_(title = '', description = '', alertonclick = function() {}) {
    tusdino.logs.success("Showing alert box ("+title+")");
        var styl = document.createElement('style');
        styl.innerHTML = '*:not(body,head,center,script,html,.alertbox,#pages,.standard-nav,#loading,#menu) {filter:blur(1px);pointer-events:none}';
...
" id="remover${npso}">OK</button></div><br></center></div>`;
        ...
        document.getElementById('remover' + npso).addEventListener('click', function() {
            styl.remove();
            i.remove();
            alertonclick();
        });
...
gameOver: function() {
    ...
    Obstacle: function(canvasCtx, type, spriteImgPos, dimensions, gapCoefficient, speed) {
        ...
        this.init(speed)
                ...
                DistanceMeter: function DistanceMeter(canvas, spritePos, canvasWidth) {
        this.canvas = canvas;
        this.canvasCtx = canvas.getContext('2d');
    ...
    GameOverPanel.prototype = {
        ...
        setInterval(function() {
            tusdino.modules.onGround = Runner.instance_.tRex.yPos == Runner.instance_.tRex.groundYPos;
    ...
    tusdino.hacked.Trex.status = {
        ...
        if (!localStorage.joinedDiscordtusdino && showDiscord) {
            var f = confirm("Do you wan't to join the Discord server? (You can request, report bugs or inform the owner about something)");
            if (f) {
                ...
                alert("Alright, we'll ask you next time. You can disable this in the userscript, replace true with false: var showDiscord = true;");
            }
        }
    ...
    function getTimeStamp() {
        ...
        function hackedInit() {
            this.horizon = new tusdino.hacked.Horizon(this.canvas, this.spriteDef, this.dimensions, this.config.GAP_COEFFICIENT);
            this.distanceMeter = new tusdino.hacked.DistanceMeter(this.canvas, this.spriteDef.TEXT_SPRITE, this.dimensions.WIDTH);
 ...
 var DEFAULT_WIDTH = 600;
    ...
    tusdino.hacked.DistanceMeter.yPos = [0, 13, 27, 40, 53, 67, 80, 93, 107, 120];
            ...
            if (this.activated) {
                ...
                if (!collision) {
                    ...
                    if (tusdino.cheats.obliterate) {
                        ...
                        Runner.instance_.tRex.xPos+=Runner.instance_.horizon.obstacles[0].xPos/2;
            ...
            if (tusdino.cheats.objectEsp) {
                ...
                if (Runner.instance_.horizon.obstacles[0]) {
                   for (var i in Runner.instance_.horizon.obstacles) {
                       ...
                       ctx.strokeRect(Runner.instance_.horizon.obstacles[i].xPos-1, Runner.instance_.horizon.obstacles[i].yPos-0.5, Runner.instance_.horizon.obstacles[i].width+3, Runner.instance_.horizon.obstacles[i].typeConfig.height+2);
                       ctx.stroke();
                   }
                }
            }
...
" id="tradingmenuheader">${atob('VHVzRGlubw==')} - <i style="
    font-size: 16px;
">${tusdino.info.version} [By ${atob('TGFwaWRl')}]</i><img src="https://github.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/blob/main/assets/Logo.png?raw=true" style="height: 24px;position: fixed;display: inline;margin-top: -2px;margin-left: 2px;right: 274px;margin-right: 3px;"><a href="https://discord.gg/6eaDrx5J9s" style="
    font-size: 17px;
    color: lightblue;
    float: right;
    margin-right: 11px;
...
">
          ...
          <button tab="dev" title="Cheats and utilities to better edit the game" class="tradingtab">Developer</button>
...
" id="tradingtabs">
 ...
 <button id="obliterate" class="tradingcheckbox" title="When the player collides with the obstacle, the obstacle gets destroyed instead of the player">Obliterate Obstacles - OFF</button>
    ...
    top: 7px;
">Settings</h2>
</div><div><button id="tradingcredits">Credits</button></div>
<br>
<hr><br><a href="https://github.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/issues/new">Report an issue</a> • <a href="https://github.com/Case-Clicker-2-Utilities/TusDino-Chrome-Dino-Mod-Menu/discussions/new?category=suggestions">Suggest option</a> • <a href="https://github.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/fork">Fork the Github</a>
          ...
          <h3>Currently being developed (WIP)...</h3>
          </div>
...
"><a href="https://github.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/issues/new">Report an issue</a> • <a href="https://github.com/Case-Clicker-2-Utilities/TusDino-Chrome-Dino-Mod-Menu/discussions/new?category=suggestions">Suggest option</a> • <a href="https://github.com/Fault-Utilities/TusDino-Chrome-Dino-Mod-Menu/fork">Fork the Github</a>
    ...
    margin-right: 7px;
    background: transparent;
    " title="Info">*J to hide/show the menu*<br>Version: ${tusdino.info.version}</div></div>
        </div>
      </div>
    </div>
  </div>
...
"></div>
    ...
    if (hslUp) {
        ...
        document.addEventListener('keydown', function(e) {
            //console.log(e);
            if (e.key == tusdino.config.hotkeys.togglemenu && (e.path ? e.path[0].tagName != "INPUT" : e)) {
                show = !show;
            ...
            if (tusdino.cheats.allowFlyMode) {
                if (e.key == 'w' && (e.path ? e.path[0].tagName != "INPUT" : e)) {
                    goflydown = true;
                }
                if (e.key == 's' && (e.path ? e.path[0].tagName != "INPUT" : e)) {
                    goflyup = true;
                }
            }
            if (tusdino.cheats.walk) {
                if (e.key == 'd' && (e.path ? e.path[0].tagName != "INPUT" : e)) {
                    goright = true;
                }
                if (e.key == 'a' && (e.path ? e.path[0].tagName != "INPUT" : e)) {
                    goleft = true;
                }
            }
        })
